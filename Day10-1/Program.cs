...
namespace Day10_1
{
    internal class Program
    {
        ...
        static Dictionary<Coord, List<Coord>> PeakMatches = new Dictionary<Coord, List<Coord>>();
        ...
        static int Explore(Coord start_pos, Coord coord, int source_height)
        {
            ...
            {
                if (PeakMatches.ContainsKey(start_pos))
                {
                    if (PeakMatches[start_pos].Any(item => item.X == coord.X && item.Y == coord.Y))
                    {
                        return 0;
                    }                    
                }
                ...
                {
                    PeakMatches.Add(start_pos, new List<Coord>());
                }

                PeakMatches[start_pos].Add(coord);
                return 1;
            }
