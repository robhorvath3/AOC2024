...
namespace Day20_1
{
    internal class Program
    {
        ...
        // finally formalized my coordinate struct (and on the day 20
        // puzzle no less!).
        ...
        static void Main(string[] args)
        {
            ...
            // local fn to remove a wall from the map and add newly
            // opened edges to graph g
            void RemoveWall(Coord pos)
            {
                map.RemoveWall(pos);
                g.AddVertex(pos);
                ...
                foreach (var dir in Enum.GetValues<Dir>())
                {
                    Coord square_in_dir = map.GetCoordInDirection(pos, dir);

                    if (square_in_dir.X != -1 && square_in_dir.Y != -1 && map[square_in_dir])
                        g.AddEdge(pos, square_in_dir);
                }
            }

            // local fn to add a wall to the map and remove the
            // appropriate edges from graph g
            ...
            // stats & counters
            int baseline_path_length = path_to_end.Count;
            HashSet<Coord> walls_removed = new();
            List<Coord> wall_saves_100_picos = new();

            // Now we need to measure the cost of removing up to 2 walls in order to find
            // paths that save more than 100 picoseconds
            foreach (Coord path_coord in path_to_end)
            {
                foreach (var dir in Enum.GetValues<Dir>())
                {
                    // see if there is a wall in this direction, and if so, see if it's
                    // a candidate to attempt removal
                    (bool remove, Coord wall_coord) = map.IsWallAndShouldRemove(path_coord, dir);

                    if (remove)
                    {
                        // make sure we haven't removed this one in a previous iteration
                        if (walls_removed.Contains(wall_coord))
                            continue;

                        // remove the wall and see if we can calculate a new path; if so,
                        // compare the runtime to the baseline
                        RemoveWall(wall_coord);

                        BFSShortestPath new_paths = new BFSShortestPath(g, map.StartPos);
                        Stack<Coord> new_path = new_paths.PathTo(map.EndPos);

                        if (new_path != null)
                            if (baseline_path_length - new_path.Count >= 100)
                                wall_saves_100_picos.Add(wall_coord);

                        // add the wall back to the map & graph and mark it as having been removed
                        AddWall(wall_coord);
                        walls_removed.Add(wall_coord);
                    }
                }
            }

            Console.WriteLine($"We removed {walls_removed.Count} walls, and of those, there were {wall_saves_100_picos.Count} that saved more than 100 picoseconds from the race");
        }
    }
}